...
function SkeletonFeatured() {
  return (
    <div className="rounded-2xl overflow-hidden border border-slate-100 bg-white" style={{ height: 288 }}>
      <div className="flex h-full">
        <div className="w-[58%] skeleton h-full" />
...
function SuggestRow({ cafe }) {
  ...
  const imgUrl = encImg(cafe.images?.[0]) || PH_IMGS[hash % PH_IMGS.length]
  const isOpen = checkOpen(cafe)

  return (
    <Link to={`/cafe/${cafe._id}`}
      className="flex items-center gap-4 p-4 bg-white rounded-2xl border border-slate-100 shadow-card hover:shadow-hover transition-all duration-250 hover:-translate-y-0.5 group">
...
export default function HomePage() {
  ...
  const searchResults = useCafes({ q: searchQuery, limit: 6 }, { enabled: isSearching })
  ...
  const FeaturedGrid = ({ data, loading }) => (
    isDesktop ? (
      <div className="grid grid-cols-3 gap-4">
        {loading
          ? [1,2,3].map(i => <SkeletonFeatured key={i}/>)
          : data?.cafes?.slice(0,3).map((cafe, i) => (
      ...
      <div className="flex gap-3 overflow-x-auto no-scrollbar pb-2 -mx-4 px-4">
        {loading
          ? [1,2,3].map(i => <div key={i} className="w-[300px] flex-shrink-0"><SkeletonFeatured/></div>)
          : data?.cafes?.slice(0,3).map(cafe => (
        ...
        <div className="max-w-7xl mx-auto px-4 lg:px-8 pt-10 pb-12 relative z-10">
          ...
          <div className={isDesktop ? 'flex items-end justify-between mb-8' : 'mb-6'}>
            <div>
              ...
              <p className="text-[14px] text-white/55 font-medium">Khám phá 240+ quán cafe chất lượng tại Hà Nội</p>
            </div>
            {isDesktop && (
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-8">
        ...
        {isSearching ? (
          <section className="pt-8 pb-6 fade-in">
            <div className="flex justify-between items-end mb-5">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h2 className="text-[18px] lg:text-[20px] font-bold text-slate-900">Kết quả tìm kiếm</h2>
                </div>
                <p className="text-[12px] text-slate-400">
                  {searchResults.isLoading ? 'Đang tìm...'
                    : `${searchResults.data?.cafes?.length || 0} quán cho "${searchQuery}"`}
                </p>
              </div>
            ...
            {!searchResults.isLoading && searchResults.data?.cafes?.length === 0 ? (
              <div className="py-16 text-center">
                <p className="text-4xl mb-4">☕</p>
                <p className="text-lg font-bold text-slate-700 mb-2">Không tìm thấy quán nào</p>
        ...
        <section className="py-8">
          <div className="flex justify-between items-end mb-5">
            <div>
              ...
              <p className="text-[12px] text-slate-400 mt-0.5">{nearby.data?.cafes?.length || 0} quán trong khu vực</p>
            </div>
            <Link to="/results" className="text-[12px] font-semibold text-blue-500 hover:text-blue-600 transition-colors">Xem tất cả →</Link>
          </div>
