...
const SOCIAL = [
  {
    name: 'Google', color: 'hover:bg-white/15',
  ...
  {
    name: 'Facebook', color: 'hover:bg-[#1877F2]/20',
  ...
  {
    name: 'TikTok', color: 'hover:bg-white/10',
    icon: <svg className="w-5 h-5" viewBox="0 0 24 24" fill="white"><path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1V9.01a6.28 6.28 0 00-.79-.05 6.34 6.34 0 00-6.34 6.34 6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.33-6.34V8.73a8.17 8.17 0 004.77 1.53V6.82a4.85 4.85 0 01-1-.13z"/></svg>,
  },
  {
    name: 'Instagram', color: 'hover:bg-white/10',
    icon: <svg className="w-5 h-5" viewBox="0 0 24 24"><defs><linearGradient id="ig2" x1="0%" y1="100%" x2="100%" y2="0%"><stop offset="0%" stopColor="#f09433"/><stop offset="50%" stopColor="#dc2743"/><stop offset="100%" stopColor="#bc1888"/></linearGradient></defs><path fill="url(#ig2)" d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>,
  },
]

/* ── Floating Label Input ── */
function FloatInput({ label, type = 'text', value, onChange, required, icon, rightSlot }) {
  ...
  return (
    <div className="relative">
      ...
      <div className={`
        ...
        ${focused
          ? 'border-cyan-400/60 shadow-[0_0_0_3px_rgba(6,182,212,0.15)]'
      ...
      `}>
        {/* Left icon */}
        {icon && (
          <span className={`absolute left-3.5 top-1/2 -translate-y-1/2 transition-colors duration-200 ${focused ? 'text-cyan-400' : 'text-white/25'}`}>
            {icon}
          </span>
        )}
        ...
        <label className={`
          ...
          ${icon ? 'left-11' : 'left-4'}
          ${active
            ? 'top-2 text-[10px] font-bold text-cyan-400'
...
export default function LoginPage() {
  ...
  function switchMode(m) {
    ...
    setSliding(true)
    setError('')
    setTimeout(() => { setMode(m); setSliding(false) }, 280)
  }
  ...
  return (
    <div className="min-h-screen flex overflow-hidden bg-slate-950">
      ...
      <div className="hidden lg:block lg:w-[55%] relative overflow-hidden">
        {BG_IMAGES.map((src, i) => (
          <div key={src} className="absolute inset-0 transition-opacity duration-1200"
            style={{ opacity: i === bgIdx ? 1 : 0, zIndex: i === bgIdx ? 2 : 1 }}>
            <img src={src} alt="" className="w-full h-full object-cover"
              onError={e => { e.target.style.display = 'none' }}/>
          </div>
        ))}
        <div className="absolute inset-0 z-10 bg-gradient-to-r from-slate-950/20 via-transparent to-slate-950/70"/>
        ...
        <div className="absolute bottom-10 left-10 z-20">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-white font-bold text-[22px]">Cafe<span className="text-cyan-400">Loop</span></span>
          </div>
          <p className="text-white/55 text-[14px] max-w-xs leading-relaxed">
            Khám phá hàng trăm quán cafe độc đáo tại Hà Nội.
          </p>
        </div>
        ...
        <div className="absolute bottom-10 right-10 z-20 flex gap-1.5">
          {BG_IMAGES.map((_, i) => (
            <button key={i} onClick={() => setBgIdx(i)}
              className={`h-1 rounded-full transition-all duration-300 ${i === bgIdx ? 'w-6 bg-white' : 'w-1.5 bg-white/30'}`}/>
          ))}
        </div>
      </div>

      {/* ── Right: form ── */}
      <div className="w-full lg:w-[45%] flex items-center justify-center bg-gradient-to-b from-slate-950 to-slate-900 px-6 py-10 relative overflow-hidden">
        <div className="absolute -top-20 -right-20 w-72 h-72 rounded-full bg-blue-500/5 blur-3xl pointer-events-none"/>
        <div className="absolute -bottom-20 -left-20 w-72 h-72 rounded-full bg-cyan-400/5 blur-3xl pointer-events-none"/>
        ...
        <div className="w-full max-w-sm relative z-10">
          ...
          <div className="lg:hidden flex items-center gap-2.5 mb-10">
            ...
            <span className="text-white font-bold text-[20px]">Cafe<span className="text-cyan-400">Loop</span></span>
          </div>
          ...
          <div className="flex gap-1 p-1 bg-white/5 border border-white/10 rounded-2xl mb-8">
            {['login','register'].map(m => (
              <button key={m} onClick={() => switchMode(m)} className={`
                ...
                ${mode === m
                  ? 'bg-white/12 text-white border border-white/15 shadow-[0_2px_8px_rgba(0,0,0,0.3)]'
          ...
          {/* Heading */}
          <div className={`mb-7 transition-all duration-280 ${sliding ? 'opacity-0 translate-y-2' : 'opacity-100 translate-y-0'}`}>
            <h1 className="text-[26px] font-bold text-white leading-tight">
              {mode === 'login' ? 'Chào mừng trở lại 👋' : 'Tạo tài khoản mới ✨'}
            </h1>
            <p className="text-[13px] text-white/40 mt-1.5">
              {mode === 'login' ? 'Đăng nhập để lưu quán yêu thích' : 'Tham gia cộng đồng café lovers Hà Nội'}
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 mb-6">
            {SOCIAL.map(({ name, icon, color }) => (
              <button key={name} className={`
                flex items-center justify-center gap-2.5 py-2.5 rounded-xl
                border border-white/10 text-[12px] font-semibold text-white/80
                bg-white/5 ${color} transition-all duration-150
                hover:border-white/20 hover:text-white hover:-translate-y-0.5 active:translate-y-0
              `}>
                {icon}<span>{name}</span>
              </button>
            ))}
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-[11px] text-white/25 font-medium">hoặc dùng email</span>
            <div className="flex-1 h-px bg-white/10"/>
          </div>

          {/* Form */}
          <form onSubmit={handleSubmit}
            className={`flex flex-col gap-3 transition-all duration-280 ${sliding ? 'opacity-0 translate-x-3' : 'opacity-100 translate-x-0'}`}>

            {mode === 'register' && (
              <FloatInput label="Họ và tên" value={name} onChange={e => setName(e.target.value)} required icon={userIcon}/>
            )}

            <FloatInput label="Email của bạn" type="email" value={email} onChange={e => setEmail(e.target.value)} required icon={emailIcon}/>

            <FloatInput label="Mật khẩu" type={showPass ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} required icon={lockIcon} rightSlot={eyeIcon}/>

            {mode === 'login' && (
              <div className="text-right -mt-1">
                <button type="button" className="text-[11px] text-white/30 hover:text-cyan-400 transition-colors">Quên mật khẩu?</button>
              </div>
            )}
