...
export default function ProfilePage() {
  ...
  useEffect(() => {
    // Lấy số yêu thích từ API
    if (user) {
      favoriteApi.list()
        .then(data => setFavCount(data.cafes?.length || 0))
        .catch(() => {})
    }
  ...
  const stats = [
    { v: user ? favCount     : 0, l: 'Yêu thích', color: 'text-rose-400' },
    { v: 0,                        l: 'Đánh giá',  color: 'text-amber-400' },
    { v: user ? historyCount : 0, l: 'Đã xem',    color: 'text-cyan-400' },
  ]

  return (
    <div className="min-h-screen w-full fade-in"
      style={{ background: 'radial-gradient(ellipse at 20% 10%, rgba(59,130,246,0.18) 0%, transparent 55%), radial-gradient(ellipse at 80% 80%, rgba(6,182,212,0.15) 0%, transparent 50%), linear-gradient(135deg,#0f172a 0%,#1e293b 60%,#0f2744 100%)' }}>
      <div className="max-w-2xl lg:max-w-3xl xl:max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 lg:py-14">

        {/* Hero card */}
        <div className="relative overflow-hidden rounded-3xl mb-8"
          style={{ background:'rgba(255,255,255,0.07)', backdropFilter:'blur(32px)', WebkitBackdropFilter:'blur(32px)', border:'1px solid rgba(255,255,255,0.15)', boxShadow:'0 8px 40px rgba(0,0,0,0.35), inset 0 1px 0 rgba(255,255,255,0.18)' }}>
            ...
            <div className="flex items-center gap-5 mb-6">
              <div className="relative flex-shrink-0">
                <div className="w-20 h-20 lg:w-24 lg:h-24 rounded-2xl flex items-center justify-center text-white"
                  style={{ background:'linear-gradient(135deg, #06b6d4, #3b82f6)', boxShadow:'0 4px 24px rgba(6,182,212,0.45)' }}>
                  {user?.name
                    ? <span className="font-black text-[32px] lg:text-[38px] leading-none">{user.name[0].toUpperCase()}</span>
                    : <svg className="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.6} d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                  }
                </div>
                <div className="absolute -bottom-1 -right-1 w-4 h-4 rounded-full bg-emerald-400 border-2 border-slate-900"/>
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-2xl lg:text-3xl font-black text-white leading-tight truncate">{user?.name || 'Khách'}</p>
                <p className="text-sm text-white/50 mt-1 truncate">{user?.email || 'Chưa đăng nhập'}</p>
                {!user && (
                  <Link to="/login" className="inline-flex items-center gap-1.5 mt-2 text-xs font-semibold text-cyan-400 hover:text-cyan-300 transition-colors">
                    Đăng nhập ngay →
                  </Link>
                )}
              </div>
        ...
        <div className="flex flex-col gap-3 mb-8">
          {MENU.map(({ icon, label, sub, to, grad, glow }, i) => (
            <Link key={to} to={to}
              ...
              style={{ background:'rgba(255,255,255,0.06)', backdropFilter:'blur(20px)', WebkitBackdropFilter:'blur(20px)', border:'1px solid rgba(255,255,255,0.12)', boxShadow:'0 4px 20px rgba(0,0,0,0.2), inset 0 1px 0 rgba(255,255,255,0.10)', animationDelay:`${i*60}ms` }}
              ...
              <svg className="w-5 h-5 text-white/25 group-hover:text-white/70 group-hover:translate-x-1 transition-all duration-300 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {user ? (
          ...
          <div className="flex flex-col items-center gap-3">
            <Link to="/login"
              className="w-full py-4 rounded-2xl text-sm font-black text-white flex items-center justify-center gap-2 hover:brightness-110 hover:-translate-y-0.5 transition-all duration-200"
